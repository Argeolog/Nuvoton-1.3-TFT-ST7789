# 1 "../driver_st7789_basic.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 391 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../driver_st7789_basic.c" 2
# 37 "../driver_st7789_basic.c"
# 1 "..\\driver_st7789_basic.h" 1
# 40 "..\\driver_st7789_basic.h"
...
# 41 "..\\driver_st7789_basic.h" 2
# 158 "..\\driver_st7789_basic.h"
uint8_t st7789_basic_init(void);
# 167 "..\\driver_st7789_basic.h"
uint8_t st7789_basic_deinit(void);
# 176 "..\\driver_st7789_basic.h"
uint8_t st7789_basic_clear(void);
# 185 "..\\driver_st7789_basic.h"
uint8_t st7789_basic_display_on(void);
# 194 "..\\driver_st7789_basic.h"
uint8_t st7789_basic_display_off(void);
# 209 "..\\driver_st7789_basic.h"
uint8_t st7789_basic_string(uint16_t x, uint16_t y, char *str, uint16_t len, uint32_t color, st7789_font_t font);
# 221 "..\\driver_st7789_basic.h"
uint8_t st7789_basic_write_point(uint16_t x, uint16_t y, uint32_t color);
# 235 "..\\driver_st7789_basic.h"
uint8_t st7789_basic_rect(uint16_t left, uint16_t top, uint16_t right, uint16_t bottom, uint32_t color);
# 249 "..\\driver_st7789_basic.h"
uint8_t st7789_basic_draw_picture_16bits(uint16_t left, uint16_t top, uint16_t right, uint16_t bottom, uint16_t *img);
# 38 "../driver_st7789_basic.c" 2

static st7789_handle_t gs_handle;
# 48 "../driver_st7789_basic.c"
uint8_t st7789_basic_init(void)
{
    uint8_t res;
    uint8_t reg;
    uint16_t i;
    uint8_t param_positive[14] = {0xD0, 0x04, 0x0D, 0x11, 0x13, 0x2B, 0x3F, 0x54, 0x4C, 0x18, 0x0D, 0x0B, 0x1F, 0x23};
    uint8_t param_negative[14] = {0xD0, 0x04, 0x0C, 0x11, 0x13, 0x2C, 0x3F, 0x44, 0x51, 0x2F, 0x1F, 0x1F, 0x20, 0x23};
    uint8_t params[64];


    memset(&gs_handle, 0, sizeof(st7789_handle_t));
    (&gs_handle)->spi_init = st7789_interface_spi_init;
    (&gs_handle)->spi_deinit = st7789_interface_spi_deinit;
    (&gs_handle)->spi_write_cmd = st7789_interface_spi_write_cmd;
    (&gs_handle)->cmd_data_gpio_init = st7789_interface_cmd_data_gpio_init;
    (&gs_handle)->cmd_data_gpio_deinit = st7789_interface_cmd_data_gpio_deinit;
    (&gs_handle)->cmd_data_gpio_write = st7789_interface_cmd_data_gpio_write;
    (&gs_handle)->reset_gpio_init = st7789_interface_reset_gpio_init;
    (&gs_handle)->reset_gpio_deinit = st7789_interface_reset_gpio_deinit;
    (&gs_handle)->reset_gpio_write = st7789_interface_reset_gpio_write;
    (&gs_handle)->delay_ms = st7789_interface_delay_ms;
    (&gs_handle)->debug_print = st7789_interface_debug_print;
    ...
    res = st7789_set_memory_data_access_control(&gs_handle, (ST7789_ORDER_PAGE_TOP_TO_BOTTOM | ST7789_ORDER_COLUMN_LEFT_TO_RIGHT | ST7789_ORDER_PAGE_COLUMN_NORMAL | ST7789_ORDER_LINE_TOP_TO_BOTTOM | ST7789_ORDER_COLOR_RGB | ST7789_ORDER_REFRESH_LEFT_TO_RIGHT));
    if (res != 0)
    {
    ...
    res = st7789_set_interface_pixel_format(&gs_handle, ST7789_RGB_INTERFACE_COLOR_FORMAT_262K,
